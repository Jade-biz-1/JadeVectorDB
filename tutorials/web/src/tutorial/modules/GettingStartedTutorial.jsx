...
import { useTutorialStateContext } from '../state_management';
import CodeEditor from '../../components/CodeEditor';
import VisualDashboard from '../../components/VisualDashboard';
import LivePreviewPanel from '../../components/LivePreviewPanel';

const GettingStartedTutorial = () => {
  const { state, actions } = useTutorialStateContext();
  ...
  const tutorialSteps = [
    {
      ...
      content: `
        ...
        <ul>
          ...
          <li>Multiple indexing algorithms (HNSW, IVF, LSH, PQ, OPQ, SQ)</li>
  ...
  const markStepCompleted = (stepId) => {
    if (!completedSteps.includes(stepId)) {
      ...
      actions.updateModuleProgress(0, completedSteps.length + 1);
    }
  };
  ...
  const completeModule = () => {
    // Mark all steps as completed if not already done
    ...
    // Unlock next module
    actions.unlockModule(1);
    
    // Add achievement
    actions.addAchievementIfNotExists({
      id: "getting_started_complete",
      title: "Getting Started Complete",
      description: "Completed the Getting Started tutorial module",
      icon: "🎓",
