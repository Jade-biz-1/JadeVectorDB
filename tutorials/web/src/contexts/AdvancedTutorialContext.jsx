...
// Create context
const AdvancedTutorialContext = createContext();

// Provider component
export const AdvancedTutorialProvider = ({ children }) => {
  ...
  return (
    <AdvancedTutorialContext.Provider value={value}>
      {children}
    </AdvancedTutorialContext.Provider>
  );
};

// Hook to use the tutorial context
export const useAdvancedTutorial = () => {
  const context = useContext(AdvancedTutorialContext);
  
  if (!context) {
    throw new Error('useAdvancedTutorial must be used within an AdvancedTutorialProvider');
  }
  
  return context;
};

export default AdvancedTutorialContext;