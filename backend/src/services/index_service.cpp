...
IndexService::IndexService() {
    logger_ = jadevectordb::logging::LoggerManager::get_logger("IndexService");
}
...
Result<Index> IndexService::get_index(const std::string& database_id, const std::string& index_id) {
    try {
        ...
        if (index_it == indexes.end()) {
            RETURN_ERROR(ErrorCode::DATABASE_NOT_FOUND, "Index not found: " + index_id);
        }
...
Result<void> IndexService::build_index(const std::string& database_id, const std::string& index_id) {
    try {
        ...
        index_it->second.updated_at = std::to_string(
...
Result<std::map<std::string, double>> IndexService::get_index_stats(const std::string& database_id, const std::string& index_id) {
    try {
        ...
        stats["dimensionality"] = static_cast<double>(index_it->second.parameters.count("dimension") ? 
            std::stoi(index_it->second.parameters.at("dimension")) : 0);
        stats["vector_count"] = static_cast<double>(index_it->second.stats ? index_it->second.stats->vectorCount : 0);
        stats["size_bytes"] = static_cast<double>(index_it->second.stats ? index_it->second.stats->sizeBytes : 0);
        stats["memory_usage_mb"] = static_cast<double>(index_it->second.stats ? index_it->second.stats->sizeBytes : 0) / (1024 * 1024);
